...
ber_analyze_file <- function(f_loc,
  ...
  whole_interval <- Intervals(c(0,endtime))
  ...
  transition_counts <- CalcTransitionCounts(state_sequence[,5])
...
compare_intersection <-  function (mother, baby, statetype=""){
  babyints <- Intervals(cbind(baby[,1], baby[,1]+baby[,2]))
  momints <- Intervals(cbind(mother[,1], mother[,1]+mother[,2]))
  mombaby_intersection <- interval_intersection(babyints, momints)
...
state_complement<- function(whole_interval, states, statetype=""){
  state_comp <- suppressWarnings(interval_difference(whole_interval, Intervals(states[,c(1,2)])))
...
find_macrostate <- function(look, touch, vocal, tag="", pt=0){
  look_ints <- Intervals(look[,c(1,2)])
  touch_ints <- Intervals(touch[,c(1,2)])
  vocal_ints <- Intervals(vocal[,c(1,2)])
  step1 <- interval_intersection(look_ints, touch_ints)
  step2 <- interval_intersection(vocal_ints, step1)
  if(length(step2)==0){
